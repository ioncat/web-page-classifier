...
|--------|-----------|---------|
| ~~**Обновить документацию под новую структуру репозитория**~~ | ✅ готово | Выполнено 14.04.2026: все пути в README.md и web/README.md обновлены под новую структуру (`pipeline/`, `data/`) |

---

## Визуальный рефреш Web UI (14.04.2026)

### Сделано

| # | Что | Файлы |
|---|------|-------|
| 80 | **Система скинов** — `data-skin` атрибут на `<html>`, `skin-switcher.js` с persist в `localStorage`, страница `/settings` с выбором скина | `web/static/skin-switcher.js`, `web/templates/settings.html`, `web/routers/pages.py` |
| 81 | **Cyberpunk skin** (архивирован) — тёмная тема с neon-бордерами. Реализовано, но не подходит для эффекта "карты на поле": тёмный фон + тёмные элементы = слабый контраст объектов | `web/static/cyberpunk.css` |
| 82 | **Minimalism skin** (активный) — светлая тема: фон `#f5f5f5`, полупрозрачные блоки `rgba(255,255,255,0.6)`, легкие тени | `web/static/minimalism.css` |
| 83 | **Structural refresh** — `body p-4` (отступ от краёв), header/sidebar/main — отдельные rounded-xl блоки с `gap-4` между ними. Убрана `sticky` у header/sidebar (были "склеены" с краями) | `web/templates/base.html` |
| 84 | **Анимированный граф на фоне** — canvas с движущимися узлами и соединениями, 100 узлов, connection-distance 200px. Реализовано, но **отключено** (закомментирован `<script>` в `base.html`) — пользователь хочет попробовать без анимации | `web/static/animated-graph.js` |
| 85 | **3 карточки в ряд** — `grid-cols-1 md:grid-cols-2 xl:grid-cols-3` в `recent/category/uncategorized/search.html` | шаблоны |
| 86 | **Header nav links** (частично) — "Новые" и "Без категории" стилизованы как pill-ссылки с padding и hover-фоном. Но суть не меняет — всё ещё "рахитские" (pending rethink) | `web/templates/base.html` |

### Запланировано

| # | Задача | Приоритет | Описание |
|---|--------|-----------|---------|
| V1 | **Переосмыслить nav в хедере** | высокий | "Новые" и "Без категории" в хедере выглядят слабо и не по делу. Возможные подходы: (1) вынести в сайдбар как отдельная секция "Быстрые виды" над категориями; (2) убрать из хедера совсем — оставить только logo + поиск; (3) сделать табы в main area над списком карточек. Решение — скорее всего перенос в sidebar или удаление из хедера |
| V2 | **Решение по анимированному графу** | средний | Сейчас отключён. Либо удалить окончательно, либо найти компромисс (более деликатная версия, fewer nodes, едва заметная) |
| V3 | **Остальные скины из плана** | низкий | Soft UI, Neumorphism — из `generic-doodling-frost.md` плана. После того как minimalism доведён до ума |
| V4 | **Решение по Cyberpunk** | низкий | Либо адаптировать под "cards on field" концепцию (светлый фон + тёмные элементы с neon), либо оставить как есть для любителей темы |
| V5 | **Бага: лейбл категории в карточке** | средний | Первоначальный запрос: при назначении категории через модалку лейбл в карточке обновляется только после перезагрузки страницы. Не решено — отвлеклись на рефреш |
| V6 | **Mobile adaptation** | средний | Minimalism тестировался на десктопе. Мобильный `#drawer`, burger — проверить и починить |
| V7 | **Финальная полировка URL-карточек** | средний | Сейчас карточки наследуют полупрозрачность от `--bg-secondary`. Возможно, карточки внутри main должны быть плотнее, чем сам main |
| V8 | **Таксономия в БД** (технический долг) | высокий | Сейчас категории живут в `pipeline/config/taxonomy.py` — добавить категорию = редактировать файл + перезапуск сервера. **Решение:** перенести таксономию в SQLite (таблица `categories`). При первом запуске — засеять из `taxonomy.py` (одноразово). Дальше и pipeline, и UI читают только из БД. Добавить страницу `/categories` в Web UI: список, форма добавления/переименования/удаления. `taxonomy.py` становится архивом. **Затрагивает:** `pipeline/db.py`, `pipeline/step3.py`, `web/database.py`, новая страница Web UI, миграция. |

### Ключевые решения

- **"Cards on field" концепция** — главный визуальный принцип: светлый фон-поле, элементы как игральные карты лежат сверху с зазорами. Тёмная тема этого эффекта не даёт (поэтому Cyberpunk отложен)
- **Анимированный граф** — опциональный эффект, не обязательный. Фон должен быть "живым", но не отвлекать от контента
- **Прозрачность блоков** — пользователь предпочёл полупрозрачные (0.6) элементы — объекты видны, но чувствуется "воздух" и фон виден насквозь

...
|--------|-----------|---------|
| **Обновить документацию под новую структуру репозитория** | P1 | Реорганизация репо (сессия 12, 13.04.2026): pipeline-файлы → `pipeline/`, данные → `data/`, docs веб-приложения → `docs/web/`. Все упоминания путей в README.md, `docs/`, `web/README.md` и `docs/web/` нужно обновить — старые пути (`config/`, `db.py`, `benchmark/`, `exports/`, `models/`, `urls.db`) заменить на новые (`pipeline/config/`, `pipeline/db.py`, `data/urls.db` и т.д.) |

...
# Размер контекстного окна (токены). По умолчанию Ollama берёт из модели (часто 128K),
# что съедает VRAM — модель уходит на CPU и работает медленно.
# Для классификации достаточно 2048–4096. Уменьшение освобождает VRAM под слои модели,
# увеличивая долю GPU (например, 29% → 67% для 24B Q4_K_M на 12GB видеокарте).
# Раскомментируйте и добавьте импорт в step3.py для активации.
OLLAMA_NUM_CTX = 2048

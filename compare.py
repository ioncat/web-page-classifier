...
Функции:
  ...
  show_comparison(limit, export)   — вывести side-by-side таблицу / CSV / XLSX
"""
import csv
import sys
import threading
...
from step3 import classify_url, get_available_models
from config.settings import (
    COMPARE_MAX_TAG_LEN as MAX_TAG_LEN,
    COMPARE_MAX_TITLE_LEN as MAX_TITLE_LEN,
    MAX_CONSECUTIVE_CONN_ERRORS as MAX_CONSECUTIVE_ERRS,
    OLLAMA_HOST,
)
...
# ── XLSX-экспорт ──────────────────────────────────────────────────────────────
def _export_xlsx(path: str, models: list[str], short_names: list[str], rows: list[dict]) -> None:
    from openpyxl import Workbook
    from openpyxl.styles import Alignment, Font, PatternFill
    from openpyxl.utils import get_column_letter

    FILL_HEADER  = PatternFill("solid", fgColor="2E75B6")   # синий
    FILL_AGREE   = PatternFill("solid", fgColor="FFFFFF")   # белый
    FILL_DISAGREE= PatternFill("solid", fgColor="FFF2CC")   # жёлтый — модели расходятся
    FILL_STRIPE  = PatternFill("solid", fgColor="F5F5F5")   # серый — чётные строки
    ...
    for row_idx, row in enumerate(rows, 2):
        cats = [row.get(m) or "" for m in models]
        disagree = len(set(c.lower() for c in cats if c)) > 1
        fill = FILL_DISAGREE if disagree else (FILL_STRIPE if row_idx % 2 == 0 else FILL_AGREE)

        title_cell = ws.cell(row=row_idx, column=1, value=row.get("title") or row.get("url") or "")
        title_cell.font      = FONT_NORMAL
        title_cell.fill      = fill
        title_cell.alignment = Alignment(wrap_text=False, vertical="center")

        for col_idx, cat in enumerate(cats, 2):
            cell = ws.cell(row=row_idx, column=col_idx, value=cat)
            cell.font      = FONT_NORMAL
            cell.fill      = fill
            cell.alignment = Alignment(horizontal="center", vertical="center")

    # ── Ширина колонок ────────────────────────────────────────────────────────
    ws.column_dimensions[get_column_letter(1)].width = 55   # Заголовок
    ...
    console.print(
        f"\n[green]Экспортировано:[/green] [bold]{path}[/bold] "
        f"[dim]({len(rows)} строк, {len(models)} моделей, "
        f"[yellow]{n_disagree} расхождений[/yellow])[/dim]"
    )
...
def show_comparison(
    ...
    export_xlsx — путь к XLSX-файлу для экспорта
    """
    init_compare_schema()
    ...
    # ── XLSX-экспорт ──────────────────────────────────────────────────────────
    if export_xlsx:
        _export_xlsx(export_xlsx, models, short_names, rows)

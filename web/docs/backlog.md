...
- [ ] `DELETE /api/urls/{id}` — удаление URL из БД
- [ ] Кнопка 🗑 на карточке: `fetch` → удаляем карточку из DOM без перезагрузки
- [ ] `PATCH /api/urls/{id}/category` — смена категории
- [ ] Drag & Drop (SortableJS): перенос карточки на категорию в sidebar
- [ ] Фолбэк для мобайла: кнопка "Переместить" → выпадающий список категорий
...
| SortableJS для DnD | Покрывает десктоп и мобайл одной библиотекой |
...
| Фаза 3 — Управление данными | 🔄 в работе |
